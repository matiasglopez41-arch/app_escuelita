...
SEED_EXCEL_PATH = Path("data/JUAGDORES_ESCUELA.xlsx")
...
def importar_excel_semilla_si_corresponde():
    if not jugadores_vacios():
        return None
    if not SEED_EXCEL_PATH.exists():
        return None

    try:
        df = pd.read_excel(SEED_EXCEL_PATH)
        validacion = validar_importacion_jugadores(df)
        if validacion["df_validos"].empty and validacion["filas_invalidas"] > 0:
            return ("error", "Se encontró el archivo semilla, pero no pudo importarse por errores de formato.")
        insertados, _, _ = importar_jugadores_desde_validacion(validacion, modo="solo_nuevos")
        return ("ok", f"Base inicial importada desde {SEED_EXCEL_PATH}. Insertados: {insertados}.")
    except Exception as e:
        return ("error", f"No se pudo importar la base inicial: {e}")
...
try:
    ...
    resultado_semilla = importar_excel_semilla_si_corresponde()
except Exception as e:
...
if resultado_semilla is not None:
    estado, mensaje = resultado_semilla
    if estado == "ok":
        st.success(mensaje)
    else:
        st.error(mensaje)
...
st.info(
    ...
    "auditoría básica, modelo preparado para multi-cliente y separación más clara entre carga inicial, operación y seguimiento."
)
